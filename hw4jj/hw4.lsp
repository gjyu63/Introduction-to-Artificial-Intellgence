...
(defun at-least-one-color (n c k)
  (cond ((or (> c k) (< k 1)) nil) ; invalid color index
        (t (cons (node2var n c k) (at-least-one-color n (+ c 1) k))))); put that prop variable into the list
...
(cond ((or (< k 1) (> c k)) nil);invalid colors
      (t (generate-clauses (at-least-one-color n c k)))))


;input: a list of clause
;output: a list of clause that is satisfied
(defun generate-clauses (l)
  (cond ((< (length l) 2) nil)
        (t (append (set-color (car l) (cdr l)) (generate-clauses (cdr l))))))

;input: a number c , output: the negation of c

(defun negate (c)
  (- 0 c))

;input: an element c, a list l, returns a list with all the combinations of -c 
(defun set-color (c l)
  (cond ((null l) nil)
        (t (append (list (list (negate c) (negate (car l))  )  ) (set-color c (cdr l))))))
...
(defun generate-node-clauses (n k)
 (cons (at-least-one-color n 1 k) (at-most-one-color n 1 k)) ;at least one and at most one
)
...
(defun generate-edge-clauses (e k)
    (cond ((or (< (car e) 1) (< (cadr e) 1)) nil)
          ((< k 1) nil)
          (t (edge-helper (car e) (cadr e) 1 k))))


(defun edge-helper (n1 n2 c k)
  (cond
    ((or (< n1 1) (< n2 1) (< c 1) (< k c) (< k 1)) nil)
    (t      (cons (list (negate (node2var n1 c k)) (negate (node2var n2 c k)))  (edge-helper n1 n2 (+ c 1) k)  ))))
