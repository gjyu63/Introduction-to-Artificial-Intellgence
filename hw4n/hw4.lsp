...
(defun at-least-one-color (n c k)
	(cond ((or (= n 0) (= c 0) (= k 0)) nil) ;invalid
			  ((> c k) nil);ends
...
(defun at-most-one-color-helper (l)
  (cond ((= (length l) 0) nil)
        ((= (length l) 1) nil)
        (t (append (set-color (car l) (cdr l)) (at-most-one-color-helper (cdr l)) ) ) ))

;negate and generate the list of combinations
(defun set-color (c l )
  (cond ((null l) nil);terminate
        (t (append (list   (cons (negate c) (list (negate (car l))))  ) (set-color c (cdr l))) )) )


(defun at-most-one-color (n c k)
  (cond ((< k 1) nil)
        ((< k c) nil)
        (t (at-most-one-color-helper (at-least-one-color n c k) ) ) ))
...
(defun generate-node-clauses (n k)
  (append (list (at-least-one-color n 1 k)) (at-most-one-color n 1 k))  );append at-least-one-color and at-most-one-color
...
(defun generate-edge-clauses (e k)
	(generate-edge-clauses-helper e 1 k)  );at most one and at least one c=1

(defun generate-edge-clauses-helper (e c k)
	(cond ((> c k) NIL)
		    (t  (append (list (list (- (node2var (car e) c k)) (- (node2var (cadr e) c k)))) (generate-edge-clauses-helper e (+ 1 c) k)))  ))
